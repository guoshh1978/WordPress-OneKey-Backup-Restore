...
if ($api_type === 'restore_status' || $api_type === 'backup_status') {
    ...
    if (!file_exists($file)) {
        echo json_encode(['success' => true, 'data' => ['percent' => 1, 'message' => '等待进程响应...', 'status' => 'init']]);
        exit;
    }
...
if ($api_type === 'restore_start' || $api_type === 'backup_start') {
    ...
    if (!$started) {
        echo json_encode(['success' => false, 'message' => '服务器不支持 exec、popen 或 curl 等后台启动方式。请联系主机提供商开启这些函数，或手动运行命令：php ' . $worker_script . ' ' . $api_type . ' ' . $task_id]);
        exit;
    }
...
if ($api_type === 'stop_task') {
    ...
    $data = json_decode(file_get_contents($file), true);
    $data['stop'] = true;
    $data['message'] = '用户请求停止任务，正在清理...';
    file_put_contents($file, json_encode($data), LOCK_EX);
    echo json_encode(['success' => true, 'message' => '停止请求已发送']);
    exit;
}

echo json_encode(['success' => false, 'message' => '无效的 api_type']);
